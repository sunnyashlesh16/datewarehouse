...
class KafkaProduceOperator(BaseOperator):
    ...
    def execute(self, context: Context) -> Any:
        ...
        for row_num in range(self.num_records + 1):
            transaction = self.generate_transaction_data(row_num)
            producer.send(self.kafka_topic, value=transaction)
            self.log.info(f'Produced row: {transaction} of {row_num}')

        producer.flush()
        self.log.info(f'Produced {self.num_records} records of transaction which has been sent to the {self.kafka_topic} topic!')